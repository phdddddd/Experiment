...
#define MEM_POOL_SIZE (1ULL << 11ULL) // 8GB: 4GB for normal traffic, 4GB for eviction
...
//     }
  ...
  char* ptr = (char*) malloc(MEM_POOL_SIZE);
  char* signalPtr = (char*) malloc(4096);
  VerbsEP* ep = connections[0];
  struct ibv_mr * mr = ep->reg_mem(ptr, MEM_POOL_SIZE);
  struct ibv_mr * signalMr = ep->reg_mem(signalPtr, 4096);
  print_mr(0, mr);

  printf("Send clients mr's addr and rkey\n");
  *(uint64_t*) signalPtr = (uint64_t) mr->addr;
  *(uint32_t*) (signalPtr+8) = mr->rkey;

  for (int i = 0; i < num_conn; i++) {
    VerbsEP* ep = connections[i];
    ep->send_signaled(1, (uint64_t)signalMr->addr, signalMr->lkey, 16);

    struct ibv_wc wc;
    while( ep->poll_send_completion(&wc) == 0){

    }
    printf("mr info is sent to connection %d. Completion status is %d\n", i, wc.status);

    ep->post_recv(1,  signalMr);
